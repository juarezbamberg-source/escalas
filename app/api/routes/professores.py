...
def criar_professor(payload: ProfessorCreate, db: Session = Depends(get_db)):
    return professores_service.criar_professor(db, payload)


@router.patch("/{professor_id}", response_model=ProfessorRead)
def atualizar_professor(professor_id: int, payload: ProfessorUpdate, db: Session = Depends(get_db)):
